...
function applyFilters(next: Partial<Filters>) {
    router.get(
        props.index_url,
        {
            category_income_id: next.category_income_id ?? props.filters.category_income_id,
    ...
    <AppShell>
        <div class="space-y-6">
            ...
            <div class="rounded-lg border bg-card">
                ...
                <div class="border-b px-4 py-4">
                    <div class="grid gap-4 md:grid-cols-3">
                        <div class="grid gap-2">
                            <label class="text-sm font-medium" for="category_income_id">Category income</label>
                            <select
                                id="category_income_id"
                                class="h-9 rounded-md border bg-background px-3 text-sm"
                                :value="filters.category_income_id ?? ''"
                                @change="applyFilters({ category_income_id: $event.target.value ? Number($event.target.value) : null })"
                            >
                        ...
                        <div class="grid gap-2">
                            <label class="text-sm font-medium" for="account_id">Account</label>
                            <select
                                id="account_id"
                                class="h-9 rounded-md border bg-background px-3 text-sm"
                                :value="filters.account_id ?? ''"
                                @change="applyFilters({ account_id: $event.target.value ? Number($event.target.value) : null })"
                            >
                                <option value="">All</option>
                ...
                <div v-if="incomes.data.length === 0" class="px-4 py-6 text-sm text-muted-foreground">No incomes yet.</div>
                    ...
                    >
                        <div class="space-y-1">
                            <div class="flex items-center gap-3">
                                ...
                                <span class="text-sm text-muted-foreground">{{ income.amount }}</span>
                            </div>
                            <div class="text-xs text-muted-foreground">
                                <span v-if="income.category_income">{{ income.category_income.name }}</span>
                                <span v-else>-</span>
                                <span class="px-1">·</span>
                                <span v-if="income.account">{{ income.account.name }}</span>
                                <span v-else>-</span>
                            </div>
                        </div>
                ...
                <div v-if="incomes.links && incomes.links.length > 0" class="border-t px-4 py-3">
