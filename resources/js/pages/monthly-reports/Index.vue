        ...
        <div class="space-y-6">
            <div class="flex items-start justify-between gap-4">
                <Heading
                    ...
                    description="Monthly expenses/incomes snapshots"
                />
            </div>
                    ...
                    >
                        <div class="space-y-1">
                            ...
                            <div class="text-xs text-muted-foreground">
                                Gastos: {{ r.total_expenses_amount }} ·
                                Ingresos: {{ r.total_incomes_amount }} ·
                                Balance: {{ r.balance_amount }}
                            </div>
                            <div class="text-xs text-muted-foreground">
                                Δ Gastos:
                                {{ r.expenses_change_amount ?? '—' }} ({{
                                    r.expenses_change_percent ?? '—'
                                }})
                                <span class="px-1">·</span>
                                Δ Ingresos:
                                {{ r.incomes_change_amount ?? '—' }} ({{
                                    r.incomes_change_percent ?? '—'
                                }})
                                <span class="px-1">·</span>
                                Δ Balance:
                                {{ r.balance_change_amount ?? '—' }} ({{
                                    r.balance_change_percent ?? '—'
                                }})
                            </div>
                        </div>
